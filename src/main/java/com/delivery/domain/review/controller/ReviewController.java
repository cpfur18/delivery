...
public class ReviewController {
    ...
    // 음식점 리뷰 목록 정렬 조회
    @Operation(summary = "음식점 리뷰 목록 조회", description = "음식점 ID를 기준으로 리뷰 목록을 정렬하여 조회합니다.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "음식점 리뷰 목록 조회 성공"),
        @ApiResponse(responseCode = "400", description = "정렬 조건이 올바르지 않음")
    })
    @GetMapping("/stores/{storeId}/reviews")
    public ResponseEntity<RestApiResponse<List<ReviewResponse>>> getReviewsByStore(
        ...
        List<ReviewResponse> response = reviewService.getReviewsByStore(storeId, sortType);
